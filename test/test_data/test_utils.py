...
def test_get_annotations_ends_no_annotation():
    ...
    df = pd.DataFrame(data, columns=['ApneaEvent'], index=tidx)
...
def test_get_annotations_ends_one_annotation():
    ...
    assert len(result) == 1
    assert result.loc['2019-01-01 00:00:03', 'ApneaEvent'] == 1
...
def test_get_annotations_ends_two_annotation():
    ...
    assert len(result) == 2
    assert result.loc['2019-01-01 00:00:03', 'ApneaEvent'] == 1
    assert result.loc['2019-01-01 00:00:06', 'ApneaEvent'] == 1
